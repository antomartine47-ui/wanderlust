...
const PUBLIC_DIR = path.join(__dirname, 'publico');
...
function readDB() {
  try {
    ...
    return {
      variantes: { A:{alpha:1,beta:1}, B:{alpha:1,beta:1}, C:{alpha:1,beta:1} },
      eventos: [],
      leads: []
    };
  }
...
function betaSample(a,b){ const x=gammaRng(a),y=gammaRng(b); return x/(x+y); }
function elegirVariante(variantes){
  const samples={};
  ['A','B','C'].forEach(v=>{ samples[v]=betaSample(variantes[v].alpha, variantes[v].beta); });
...
app.get('/api/leads', (req,res)=>{ const db=readDB(); res.json(db.leads.reverse()); });
...
// Servir cualquier .html que pidan (Acerca-de.html, Destino.html, etc)
app.get('*.html', (req,res)=>{
  const file=path.join(PUBLIC_DIR, req.path);
  if(fs.existsSync(file)) res.sendFile(file);
  else res.sendFile(path.join(PUBLIC_DIR,'index.html'));
});
